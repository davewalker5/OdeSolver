...
def classify_winter_model(
    ...
    warnings: list[str] = []
    validate_month("WINTER_PEAK", p["WINTER_PEAK"], warnings)
    validate_month("AUTUMN_PEAK", p["AUTUMN_PEAK"], warnings)
    validate_month("SUMMER_LOW", p["SUMMER_LOW"], warnings)
    ...
    autumn_ratio = safe_ratio(p["AUTUMN_WEIGHT"], p["WINTER_WEIGHT"])
    decay_growth_ratio = safe_ratio(p["DECAY_RATE"], p["GROWTH_RATE"])
    ...
    return {
        ...
        "derived_metrics": {
            "winter_peak_month": decimal_to_float(p["WINTER_PEAK"]),
            "winter_peak_label": month_label(p["WINTER_PEAK"]),
            "autumn_peak_month": decimal_to_float(p["AUTUMN_PEAK"]),
            "autumn_peak_label": month_label(p["AUTUMN_PEAK"]),
            "summer_low_month": decimal_to_float(p["SUMMER_LOW"]),
            "summer_low_label": month_label(p["SUMMER_LOW"]),
            "autumn_to_winter_weight_ratio": decimal_to_float(autumn_ratio),
            "decay_to_growth_ratio": decimal_to_float(decay_growth_ratio),
            ...
            "monthly_target": {MONTH_NAMES[m]: decimal_to_float(v) for m, v in monthly_target.items()},
        },
        "parameter_evidence": {
...
def _build_summary(
    ...
    return (
        ...
        f"The fitted winter component peaks around {month_label(winter_peak)}, "
        f"with a {autumn_component} autumn component centred around {month_label(autumn_peak)}. "
        f"The model has {baseline_presence} baseline presence and {summer_suppression} summer suppression "
        f"centred around {month_label(summer_low)}. "
